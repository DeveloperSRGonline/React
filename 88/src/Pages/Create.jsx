...
const Create = () => {
  ...
  const submitHandler = (recipe) => {
    ...
    // Convert string to array (like in SingleRecipe)
    recipe.ing = recipe.ing.split(",").map((i) => i.trim());
    recipe.inst = recipe.inst.split(",").map((i) => i.trim());
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-around mb-5">
        <button className=" bg-[#f95941] text-white px-16 py-4 font-bold text-2xl rounded  w-[90%] mx-auto  active:scale-[0.97] duration-200">
          Save Recipe
        </button>
      </div>
    </form>
